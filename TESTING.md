...
| --- | --- | --- |
| [`test/routes/event-overlap.test.ts`](test/routes/event-overlap.test.ts) | Event CRUD, overlap rules, optimistic versioning, concurrent requests, owner isolation, list filters, title search, and iCalendar export | Fastify route plugin + real in-memory MongoDB. Each test clears the collection and seeds one Alice event. |
...
| Command | Result |
| --- | --- |
| `bun run test` | 85 passed, 0 failed across 7 files; 97.32% line coverage and 92.03% function coverage. |
| `bun run compile` | Passed for application and test TypeScript. |
| `bunx biome ci test/mongo.test.ts test/routes/event-overlap.test.ts --reporter=summary --colors=off` | Passed for the currently staged test files. |
| `bun run check` | Does not currently pass: `test/init-mongo.test.ts` and `test/options.test.ts` need formatting. |
